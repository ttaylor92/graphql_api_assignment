defmodule GraphqlApiAssignment.ResolverBucketTest do
  ...
  describe "incrementing user action counts" do
    test "increments create_user count" do
      {:ok, pid} =
        ResolverBucket.start_link(name: :create_user_bucket, table_name: :create_user_table)
    ...
    test "increments update_user count" do
      {:ok, pid} =
        ResolverBucket.start_link(name: :update_user_bucket, table_name: :update_user_table)
    ...
    test "increments update_user_preferences count" do
      {:ok, pid} =
        ResolverBucket.start_link(
          name: :update_user_preferences_bucket,
          table_name: :update_user_preferences_table
        )
    ...
    test "increments get_users count" do
      {:ok, pid} =
        ResolverBucket.start_link(name: :get_users_bucket, table_name: :get_users_table)
